...
def register_user(email, password, display_name):
    ...
    try:
        check_user_exists = (
            db.collection("users").document(display_name).get().to_dict()
        )
        ...
        if check_user_exists == None:
            ...
            st.success(
                f"Successfully registered user {display_name} with email {email} !"
            )
            return user
        ...
        st.error(
            f"{e}, if you have already registered on **Setu Dashboard**, please login"
        )
        return None
...
def login_user(email, password):
    ...
    """
    url = "https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=key_here"
...
if st.session_state["authentication_status"] == True:
    try:
        ...
        user_data = (
            db.collection("users")
            .document(st.session_state["username"])
            .set({"n_urls_verified": 0, "url_ids_verified": []})
        )
